...
function Chase(RefID){
    document.forms[0].ReferralID.value = RefID;
    document.forms[0].action = 'sysadmin/chasegatekeeper'
    document.forms[0].submit(); 
}
</script>
<?=(isset($Message) and !$Message==''?'<p><b style="color:red;"><?=$Message;?></b></p>':'');?>
<fieldset><legend>All Current Referral</legend>
<form id="myform" method="post">
   ...
   <input name="ReferralID" type="hidden" value="" />
   <table class="report" width="60">
       <tr><td colspan="8">Below is a list of locations where you are currently 
      ...
      <tr>
         ...
         <th style="cursor:pointer;" onclick="ReOrder('Status');" colspan='2'>Status</th>
      </tr>
      <tr>
         <td>ID</td>
         <td>
            <select name="Country" onchange="this.form.submit();" width="20" style="width:80px;" size="0">
         ...
         <td>
            <select name="Province" onchange="this.form.submit();" width="20" style="width:80px;" size="0">
               <option value=""></option><?php
            if(isset($Provinces)){
         ...
         <td>
            <select name="City" onchange="this.form.submit();" width="20" style="width:80px;" size="0">
               <option value=""></option><?php
            if(isset($Cities)){
         ...
         <td nowrap>
            <select name="District" onchange="this.form.submit();" width="20" style="width:80px;" size="0">
               <option value=""></option><?php
            if(isset($Districts)){
          ...
          <td nowrap><select name="Returning" onchange="this.form.submit();" width="20" style="width:80px;" size="0">
                 <option></option><?php 
...
<?php foreach($results as $row):?>
      <tr>
	    ...
	    <option value="Expired"<?=($row['Status'] == 'Expired'?' Selected':'');?>>Expired</option>
         ...
         <td><?php
            if(!in_array($row['Status'],array('Declined','Chased','CityWatch',
                    'Confirmed','Connected','Failed','Cancelled','Concerned')) 
                and strtotime($row['Updated']) < strtotime("1 month ago")){?>
             <input type="button" value="Chase" width="3"
            onclick="Chase(<?=$row['ReferralID'];?>);"/><?php
             }?>
         </td>
