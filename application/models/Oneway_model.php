...
class oneway_model extends CI_Model {
    private $websiteURL  = 'https://www.cwisw.org.uk/';
    private $websiteRoot = 'oneway/';
      ...
      if($row){
         if($row['Entered'] == $row['Password']){
            ...
            if(isset($data['MemberID'])){ 
                ...
                case 'concerns': 
                    $data['Menu']='member'; 
                    $data['NextPage'] = "member/member"; break;
                default:
                    $data['Menu'] = strtolower($row['Status']);
                    $data['NextPage'] = $data['Menu'].'/'.$data['Menu'];
                endswitch;
                    
              
  //             $data['NextPage'] = $data['Menu'].'/'.$data['Menu']."home";
               $sql = "UPDATE members "
    ...
    public function Logout($data){
        ...
        //unset_userdata('MemberID');
        //unset_userdata('FirstName');
        //unset_userdata('LastName');
        //unset_userdata('Status');
        //unset_userdata('Account');
        //unset_userdata('Email');
        //unset_userdata('C2P_Mobile');
        unset($data['MemberID']);
